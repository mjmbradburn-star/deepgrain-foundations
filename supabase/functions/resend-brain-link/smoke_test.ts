...
const SERVICE_ROLE_KEY = Deno.env.get("SMOKE_SERVICE_ROLE_KEY") ??
  Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;

assert(
  SUPABASE_URL,
  "Missing SUPABASE_URL (set SMOKE_SUPABASE_URL or SUPABASE_URL)",
);
assert(
  SERVICE_ROLE_KEY,
...
async function cleanup(email: string, subscriberId?: string): Promise<void> {
  if (subscriberId) {
    await admin.from("brain_access_tokens").delete().eq(
      "subscriber_id",
      subscriberId,
    );
  }
  ...
  async () => {
    ...
    try {
      ...
      assert(
        ...
        `expected 1 active token, got ${tokens?.length}`,
      );
      const token = tokens[0].token as string;
      assert(
        token.length === 64,
        `expected 64-char hex token, got ${token.length}`,
      );
      ...
      assertEquals(
        open.status,
        302,
        `open-brain body: ${open.text.slice(0, 200)}`,
      );
      assert(
      ...
      assertEquals(
        active.length,
        1,
        "exactly one active token after second resend",
      );
      assert(
