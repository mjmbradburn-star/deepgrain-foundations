...
async function logOutcome(
  ...
  try {
    ...
    console.warn('resend-brain-link: metric persist failed', { outcome, err: String(err) })
  }
}
...
Deno.serve(async (req) => {
  ...
  if (req.method !== 'POST') {
    await logOutcome(null, 'resend_method_not_allowed', { ip, detail: { method: req.method } })
  ...
  if (rateLimited(ip)) {
    ...
    return jsonResponse(
      { status: 'rate_limited', error: 'Too many requests. Please try again in a minute.' },
      429,
    )
  }
  ...
  try {
    ...
    await supabase
      ...
      .update({ revoked_at: new Date().toISOString(), revoke_reason: 'resend_superseded' })
    ...
    if (tokenInsertError) throw new Error(`token_insert_failed: ${tokenInsertError.message}`)
    ...
    return jsonResponse({ error: 'Something went wrong while sending. Try again shortly.' }, 500)
  }
})
