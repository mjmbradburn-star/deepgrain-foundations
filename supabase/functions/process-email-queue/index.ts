...
function normalizeFromAddress(from: unknown): string {
  if (typeof from === "string" && from.trim()) {
    ...
    if (withName) {
      ...
      return name
        ? `${name} <${local}@${VERIFIED_FROM_DOMAIN}>`
        : `${local}@${VERIFIED_FROM_DOMAIN}`;
    }
...
async function sendResendEmail(
  ...
  if (
    ...
    const unsubscribeUrl = `${opts.unsubscribeBaseUrl}?token=${
      encodeURIComponent(payload.unsubscribe_token)
    }`;
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (body?.message) {
        message = `Resend API error ${res.status}: ${body.message}`;
      }
    } catch {
