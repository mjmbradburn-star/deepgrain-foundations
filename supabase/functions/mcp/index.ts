...
var search_intelligence_default = defineTool({
  ...
  inputSchema: {
    ...
    limit: z.number().int().min(1).max(20).optional().describe(
      "Maximum number of matching articles to return. Default 8.",
    ),
  },
  annotations: {
    readOnlyHint: true,
  ...
  handler: async ({ query, limit }) => {
    ...
    if (!res.ok) {
      return {
        content: [{
          type: "text",
          text: `Failed to fetch corpus: ${res.status}`,
        }],
        isError: true,
      };
    }
  ...
  handler: async ({ urlOrSlug }) => {
    ...
    if (!res.ok) {
      return {
        content: [{
          type: "text",
          text: `Failed to fetch ${target}: ${res.status}`,
        }],
        isError: true,
      };
    }
...
var mcp_default = defineMcp({
  ...
  tools: [
    ...
    fetch_article_default,
    list_pillars_default,
  ],
});
