...
export const faqs = [
  ...
  },
  {
    ...
    answer:
      "Because two companies can run the exact same frontier model and get completely different results, purely on the strength of what surrounds it. Watch what a company spends its AI budget on first: if it is a model upgrade rather than data cleanup or tool wiring, the outcomes will not move much.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Readiness is the diagnostic. The AI operating system is the runtime. Teams that only ever run the diagnostic tend to re-score the same five pillars every quarter without building anything, because nobody moved from grading to construction. The score is only useful if a build follows it.",
  },
];
