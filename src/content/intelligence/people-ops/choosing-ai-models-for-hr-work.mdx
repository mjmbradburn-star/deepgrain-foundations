...
export const faqs = [
  {
    ...
    answer: "No. Running two or three subscriptions costs about £15-20 a month each, less than the time it takes to fix one bad first draft. The default stack: ChatGPT for structured drafting and workflows, Claude for nuanced writing and policy work, Perplexity or Gemini for research with citations. Most teams stick to one model out of habit, then wonder why the writing never quite lands.",
  },
  {
    question: "Which model should we use for writing HR policies?",
    answer: "Claude wins on policy drafting because it handles tone, edge cases, and the implicit \"what would a reader take from this\" check. GPT is fine for structured first drafts. Whichever model writes it, run the draft through a second model for critique. Two passes, two perspectives, far fewer mistakes.",
  },
  {
...
<Takeaway>Most teams run one model because nobody decided otherwise. A reasonable People Ops stack: ChatGPT for structured drafting, Claude for nuanced writing and policy, Perplexity or Gemini for sourced research.</Takeaway>
...
Two reliable habits sit underneath this stack: never ship a first draft, and always cross-check important outputs with a second model. The cost is one extra prompt, one extra pass. The pay-off is work that doesn't need redoing.

## What this means in practice

Most teams that run one model never chose to. They signed up for one in year one and never revisited it. Four tabs open, four logins, that is the actual friction cost of switching between Claude, ChatGPT, Gemini and Perplexity now, and it is close to zero. The right answer for almost every team is "use the right one for the task."

That is also one of the easier early wins for the [champion model](/intelligence/the-champion-model). Champions can document, by task type, which model the team should reach for, and embed that into the [AI workspace](/intelligence/setting-up-your-ai-workspace). It's a one-page doc. Most teams never write it, and pay for that every time someone drafts a redundancy letter in the wrong model.
