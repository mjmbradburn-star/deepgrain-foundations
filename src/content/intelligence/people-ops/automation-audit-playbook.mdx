...
export const faqs = [
  {
    question: "Why do 90% of automation efforts fail to deliver expected ROI?",
...
The audit is the bridge between the [diagnostic](/intelligence/diagnosing-ai-readiness-in-people-ops) and the build. Without the diagnostic, you do not know what the function can absorb. Without the audit, you do not know what to build. Without the [operating model](/intelligence/ai-enablement-operating-model), the wins do not compound.

Once a workflow has been audited and selected, the build pattern follows the move from [prompts to systems](/intelligence/from-prompts-to-systems). The most successful builds, in our experience, hold to the same problem-first discipline that the audit produced. Tools change. The discipline does not.

If you do nothing else, change the question. From "what can I automate?" to "what problems am I trying to solve?" That single shift is worth more than any specific framework on top of it.
