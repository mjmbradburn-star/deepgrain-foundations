...
export const faqs = [
  ...
  },
  {
    ...
    answer: "Org-wide is the unglamorous backbone: standards, guardrails, tooling strategy, capability expectations, governance, measurement, incentives. Team-wide is where the work actually happens: workflow transformation, shared assets, an operating cadence. Individual is the human layer: workspace, context, output standards, evaluation habits. Skip one and the other two stall. Rituals with no org backbone give you inconsistent standards team by team. Individual skill with no team cadence gives you isolated power users the rest of the function can't learn from.",
  },
  {
    question: "What does \"good\" look like in the first 90 days?",
    answer: "Three to five use cases with a number attached to each: hours saved, error rate, cycle time. One published policy page. A weekly show-the-thing cadence nobody needs reminding about by month two. The real test: could a sceptical manager on a completely different team name one thing that's changed. If the answer's no, you've built a project, not a capability.",
  },
  {
...
The gap sits one level up from the tools. AI enablement needs to be treated as a company capability, comparable to leadership development, security awareness, or management fundamentals: three connected layers, not one.
...
Champions are a distribution layer, not a strategy. Build the backbone underneath them and the same champions stop restarting from zero every time someone rotates off the programme.
...
- **Workflow transformation.** Each team picks its top three workflows and rebuilds them with AI in the loop. Take onboarding: redesign the whole flow so AI sits at the moments that actually slow people down, day one, week two, first review, rather than being bolted onto induction as an add-on. The before-and-after is visible enough that new hires notice it themselves.
...
The CHRO who owns this work is doing something specific: building a system that turns AI into a sustained capability of the function, then the company. Licenses and training programmes are inputs to that, not the deliverable.
