...
export const faqs = [
  {
    ...
    answer: "Because most teams fall into one of two traps, dabbling or tool-shopping, and both look like progress from the outside. The tell is simpler than either trap: ask who owns the workflow that was supposed to change. If nobody can answer in one sentence, nothing has actually moved, whatever the deck says.",
  },
  {
    question: "What is the difference between a workflow, an automation and an agent?",
    answer: "A workflow has humans deciding at the hard points. An automation runs the same rule every time with nobody watching. An agent decides what to do next on its own, inside guardrails you set. Quick test: if you can finish the sentence \"if X happens, do Y\" without adding \"it depends,\" you've got an automation, not an agent. Most teams reach for an agent when they haven't actually finished that sentence yet.",
  },
  {
    question: "Which one should we start with?",
    answer: "Workflows, almost always. And it doesn't take long. Documenting one properly, start to finish, decision points marked, is usually a single working session, not a project. Automate the rule-based parts inside it. Only reach for an agent once the process is stable enough that a new starter could follow it from the doc alone.",
  },
  {
    question: "What does a real AI capability inside a People function actually look like?",
    answer: "Four things, held together: context the system actually holds rather than context you retype every time, workflows the team can re-run and trust, a handful of internal builders who can wire tools together, and governance that says where the human stays in the loop. In practice the piece missing most often is the builders. Teams collect context and sketch workflows just fine. They stall because nobody inside the team can actually build the thing.",
  },
  {
    question: "What is the third path?",
    answer: "A small group inside the team rebuilds one painful, well-bounded workflow with AI in the loop, watches where it breaks, fixes it, then moves to the next. It stalls in one predictable way: picking a first workflow that's too big or too political to fail safely on. Pick something where a bad first attempt costs an afternoon, not a quarter, and the rest of the pattern takes care of itself.",
  },
];
...
- Systems thinking is the shift that matters. The prompt is a small part of it.
</TLDR>

The technology is cheap. The curiosity is high. The leadership pressure is real. And six months in, almost nothing has changed in how the work actually gets done. Recruiters still triage manually. Onboarding still depends on a single person's calendar. Performance reviews still consume entire weeks every cycle. The dashboard says "AI initiative in progress." The grain says otherwise.
...
A real AI capability inside a People function looks like four things, running together:
...
The third path has a shape. Not "use more AI." A sequence: workflows first, automations second, agents only when both are in place.
...
Automations work when the rules are stable, the inputs are clean, and judgment adds nothing. They fall apart when the situation needs context, when policies vary, or when the rules change every quarter.
...
The shift from tasks to outcomes is the shift from doing AI to building with AI. It also makes the work measurable, and measurable is what survives a budget conversation.
...
After six months, the team hasn't ticked a box marked "done AI." Four pieces of its work now run differently. The change compounds. People outside the team start asking how. The People function, quietly, becomes the part of the company most fluent in building with AI.
