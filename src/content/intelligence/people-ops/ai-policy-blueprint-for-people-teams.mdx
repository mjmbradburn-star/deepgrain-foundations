...
export const faqs = [
  {
    ...
    answer: "Two reasons. They ban instead of enable, so people route around them inside a week. And Legal writes them alone, without People, IT, or the people who will actually use the tool, so the policy describes a workflow that does not exist. Ask when anyone last opened the policy on purpose, not during onboarding. If nobody can answer, that is the failure mode.",
  },
  {
    question: "What about shadow AI? Around 71% of employees use unapproved tools.",
    answer: "Banning does not solve it, it hides it. The tool is rarely the problem, what goes into it is. Someone pasting a real employee's performance review into a consumer chatbot is the actual risk, not the fact that they reached for ChatGPT instead of the approved one. Find out what people are using and why, then treat every unapproved tool as a vote against your current approval process.",
  },
  {
...
**Assemble a cross-functional working group.** Voices from People, Legal and Compliance, IT and InfoSec, Data Protection, DEI, and crucially, employee representatives who can speak to real workflow pain, plus the tech-savvy managers who will actually use the policy day to day. The group meets regularly during rollout and quarterly once operational. Their job is to enable safe innovation, not block it entirely.
...
Foundation done, draft the policy. The structure that consistently works:
