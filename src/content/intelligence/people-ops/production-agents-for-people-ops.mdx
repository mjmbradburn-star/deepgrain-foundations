...
export const faqs = [
  {
    ...
    answer: "A system that takes a goal, decides the next action, calls tools, keeps state, and leaves an audit trail. A Zendesk macro that fires back canned text isn't one. A Zapier zap that forwards an email isn't one either. The test: can it decide what to do next on its own, and can you see why afterwards? Miss either and it's automation wearing an agent costume, not a junior operator with superpowers.",
  },
  {
    question: "Why do most agents fail in production?",
    answer: "Lack of context, lack of state, lack of guardrails. Most teams spend 80% of their time choosing the model and 20% on what the agent actually needs to succeed. The ratio should be reversed. The model is a tuning decision. Context, state, tooling contracts, exception handling, observability, and human escalation are what separate an agent worth real money from one worth nothing.",
  },
  {
    question: "Why does data matter more than apps for HR automation?",
    answer: "Because features don't fix a discovery problem. HRIS, ATS, payroll, performance, and engagement systems don't share a language, so nobody has one clean view of an employee. Wire that up with n8n or similar and you get more capability than the tenth SaaS tool ever ships. Rule of thumb: if you can't pull the data through an API today, don't build an agent on top of it tomorrow.",
  },
  {
    question: "Where should we start with agents in People Ops?",
    answer: "Not with agents. Start with workflows you can document and automations that already run clean. Once the data feeding them is trustworthy, pick one bounded use case, a policy FAQ bot, an onboarding step-trigger, a comp research assistant, and give it a real escalation path from day one. Don't expand scope until it has handled real cases for a few weeks without you babysitting it.",
  },
];

<TLDR>
- A production agent has state, evals, and an owner. A prompt with ambition has none of those.
...
This is fixable. The agents that survive production share a pattern. The ones that fail share the opposite one. Get it right and the automation programme compounds. Get it wrong and it quietly gets retired.
...
The reflex is wrong. The data sits in silos, not the features. The HRIS knows the employee. The ATS knows the candidate. Payroll knows the comp. Performance knows the rating. Engagement knows the sentiment. None of them know each other.

Buying a tenth tool adds a tenth silo. The real value sits in the integration and orchestration layer.
...
A useful test: when something breaks, can you see which step failed? With a connected workflow built in n8n, the answer is yes. With a black-box vendor product, often no. Visibility is what lets you fix things fast. Speed is what lets you scale.
...
Miss any of those and what you have built is a roulette wheel with a UI.
...
The pattern across all of them is the same: move from [prompts to systems](/intelligence/from-prompts-to-systems), use the [domain map](/intelligence/the-people-ops-ai-domain-map) to point the work at the right target, and lean on the [operating model](/intelligence/ai-enablement-operating-model) to make it survive a quarter.

Production agents are not magicians. They are operators. Build them like operators and they work like operators.
