...
export const faqs = [
  {
    ...
    answer: "Nearly every AI failure inside a People function turns out, on the postmortem, to be a change failure rather than a technology one. The tools worked, the workflow was sound. What actually broke is always the same three things: a fear that was never named out loud, a rollout paced faster than the team could absorb, and a leader who announced the change instead of running it. Ask any CPO who has been through a failed rollout what they would do differently, and none of them mention the tool stack.",
  },
  {
    question: "How should a CPO open the conversation about AI with the team?",
    answer: "Name the fear before the team gets there themselves. \"This will free us up for more strategic work\" invites the silent rebuttal: or to do the same work with fewer of us. Say instead that AI will change what the team does, that some roles will look different, that capability will grow faster than the work shrinks, and that nobody will be blindsided by a change to their own role. It has to come from the CPO directly, in person, not a slide a comms team wrote, and it has to happen before a single tool gets rolled out, not after.",
  },
  {
    question: "What is the sequence that actually works?",
    answer: "Five phases, in order: personal capability first (months 1-2, the CPO's own reps before anyone else's), one small visible win (months 2-4), champions plus a shared workspace (months 3-6), function-wide rollout (months 6-12), then redesign (months 12+). The phases overlap but skipping one always costs more later - skip Phase 1 and the failures get expensive fast. Functions that have tried compressing this into ten weeks have not pulled it off yet, not once.",
  },
  {
    question: "What should we measure to know the transformation is working?",
    answer: "Not usage. Prompts, active users, training hours logged - none of it tells you who is actually faster because of the work, only who is busy. Three numbers are worth tracking: workflow count and depth (how many workflows run end to end on AI infrastructure, and how many survive the original builder leaving), time reallocation (has coordination and drafting time actually fallen, has judgment and partnering time actually risen), and confidence (can the team teach a new joiner how it uses AI). If workflow count has not moved by month nine, the transformation has stalled - full stop.",
  },
];
...
This is the part most CPOs find hardest. Not the AI - the leadership: a transformation run while the rest of the business is also changing, with a team that is rightly anxious about what it means for them.

There is a sequence that works, though. Nothing novel about it, the same sequence that works for any serious change. It just has to be done - properly, and in order.

<Takeaway>AI in People Ops fails as a change programme more often than as a technology problem. Name the fear, sequence the moves, address resistance honestly.</Takeaway>
...
**Phase 2: A small visible win (months 2–4).** Pick one workflow. Bounded. Painful. Visible. Build it. Ship it. Tell the story of what changed and why. Not a pilot deck. An actual thing the team is using on a Wednesday. The point: proof, concrete, that this works inside *our* team, with *our* tools, on *our* problems.
...
Resistance is information, not a problem to eliminate. Four patterns recur, and each has a response that works.

**The Sceptic.** *"I tried it once, it was wrong, this is overhyped."* Usually a senior, experienced operator who is not wrong about much. The response is a one-on-one demonstration of a real workflow that solves a problem they actually have, not a deck. Sceptics convert when they see the workflow solve their own problem, in their own hands; they never convert from a presentation.
...
Resist the urge to measure usage. Number of prompts, number of users, hours of training delivered - these are vanity numbers. They tell you who is busy, not who is actually faster because of it.
