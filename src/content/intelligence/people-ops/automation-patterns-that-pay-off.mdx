...
export const faqs = [
  ...
  },
  {
    ...
    answer:
      "You do not need engineers. You need a champion, a workflow tool, and one clean process. The usual stack is a workflow tool for the steps (around twenty pounds per builder seat a month, SOC 2 and ISO 27001, self-hostable), an LLM with enterprise terms for the judgement, your HRIS as source of truth, and Slack or email as the surface. A trained person on the team builds each pattern in days or weeks.",
  },
  {
...
The stack that makes these patterns work is deliberately unglamorous, and it is the same one the wider [People Ops AI system](/intelligence/pillar/people-ops-ai) is built on. A workflow tool: self-hosted if you want control, managed if you want less upkeep, the simplest one on the market if you want zero learning curve. A good one runs about twenty pounds per builder seat a month, is SOC 2 and ISO 27001 compliant, and self-hosts if your security team needs it inside your own perimeter. Then an LLM provider with enterprise terms for the judgement steps, your HRIS as the single source of truth, and Slack or email as the surface where humans review and approve. That is enough to build every pattern below. There is no missing sixth tool.
...
<ProcessFlow
  stages={[
    ...
    { label: "Wire the tools", body: "HRIS as source of truth, a workflow tool for the steps, an LLM for the judgement, Slack as the surface.", meta: "Week 1" },
