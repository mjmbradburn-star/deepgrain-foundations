export const frontmatter = {
  ...
  keywords: ["n8n automation HR", "People Ops workflows", "AI automation patterns", "HR process automation", "AI operational efficiency", "AI business efficiency"],
  readTime: "11 min",
...
export const faqs = [
  {
    question: "How do businesses use AI agents to improve efficiency?",
    answer:
  ...
  {
    question: "What AI automation patterns actually pay off?",
    answer:
      "Six recur across People functions: inbound triage, interview scorecard summarisation, first-week onboarding sequencing, manager check-in cycles, policy and handbook Q&A, and performance cycle preparation. Start with triage and onboarding, they're the easiest to scope and the fastest to show visible value. Each is a single workflow with one human checkpoint, not a bespoke agent.",
  },
  {
    question: "How does AI improve operational efficiency in businesses?",
    answer:
      "It makes the boring repeatable work faster and makes the disagreement inside that work visible. Smoother weeks matter more than headline savings: fewer dropped onboardings, sharper interview debriefs, manager check-ins that actually happen. The pattern compounds across hires and cycles.",
  },
];
...
Six patterns. That's most of what AI actually does inside a People function, once you strip away the roadmap decks and the agent hype. Here they are, in the order most teams should build them, with the tools that make them work.

The stack we recommend: a workflow tool (n8n if you want self-hosted, Make if you want managed, Zapier if you want zero learning curve), an LLM provider with enterprise terms, the company's HRIS as source of truth, and Slack or email as the surface where humans interact. That is enough.

<Takeaway>Six concrete People Ops automation patterns. Built with n8n, an LLM, and a champion. Live in weeks, not quarters.</Takeaway>
...
Critical detail: the workflow surfaces and prepares, nothing more. The recruiter clicks. The audit log captures everything.
...
This is where AI earns its keep - making the disagreement legible before the room ever sits down.
...
This is the workflow that most directly changes the culture. Visibility is its own intervention.
...
The hard part is keeping the handbook actually current. Get that wrong and the assistant answers confidently from stale policy. AI surfaces the documentation hygiene problem you already had.
