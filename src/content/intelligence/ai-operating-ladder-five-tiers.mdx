...
export const faqs = [
  {
    ...
    answer:
      "The AI operating ladder is a five-tier model of AI operating maturity used at Deepgrain: ad-hoc, assisted, augmented, autonomous, and self-operating. It came out of running the same 'where are we, really' conversation with enough leadership teams that a single maturity score stopped being useful. Naming the rung per function, rather than an aggregate number, is what the AI Operating Index scores against.",
  },
  {
    question: "Which tier is most companies on?",
    answer:
      "It varies sharply by function. Customer support and marketing tend to move fastest because the cost of a bad output is low and reversible. Finance and legal lag hardest because the stakes of an error are higher, so governance has to catch up before the tooling does. Scoring function by function, rather than the org as a whole, is what usually breaks the leadership-versus-operator standoff.",
  },
  {
...
Specific outcomes, not just specific tasks. An agent owns the customer-reply backlog: it drafts the first-pass reply, escalates the unclear ones, and reports on the rest. A human reviews the policy and the exceptions, not the volume.
