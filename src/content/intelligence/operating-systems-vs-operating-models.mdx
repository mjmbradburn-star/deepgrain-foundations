export const frontmatter = {
  title: "Operating system vs operating model, with 3 real examples",
  slug: "operating-systems-vs-operating-models",
  category: "foundations",
  description: "An operating model is the slide. An operating system is what runs under pressure. See the gap in promotions, onboarding and enquiries, and how to read it.",
  keywords: ["operating system vs operating model", "operating model vs operating system", "operating model", "operating system", "AI operating model", "organisational design"],
  readTime: "10 min",
...
export const faqs = [
  {
    question: "What is the difference between an operating model and an operating system?",
    answer:
  ...
  {
    question: "Why do consulting projects change the operating model but not how the company actually runs?",
    answer:
  ...
  {
    question: "How do I read my company's operating system?",
    answer:
  ...
  {
    question: "What is an AI operating model versus an AI operating system?",
    answer:
...
<TLDR>
- An operating model describes intent. An operating system describes what actually runs when nobody is looking. That is the whole distinction.
- Every company has both, and the gap between them is where most consultancies make their money and where most of the value is lost.
- Leaders reach for the model because it can be drafted in an afternoon. The system cannot, because it was never drafted; it accretes one workaround at a time.
- AI has reproduced the same gap: an AI operating model is a deck, an AI operating system is what quietly runs a workflow overnight.
- Read the system before you redesign the model. A model built on an unread system just stacks a second fiction on the first.
</TLDR>
...
An operating model describes how work is meant to move. An operating system is how work actually moves under a real deadline: the habits, hand-offs and workarounds people use when nobody is watching. Both matter. Changing the model without reading the system is why expensive redesigns leave the floor untouched.
...
<ComparePanel
  a={{ heading: "The operating model", items: [
  ...
  b={{ heading: "The operating system", items: [
  ...
  caption="Both are real. Only one is running the business."
/>
...
<Takeaway>A model is what people intended. A system is what they do under pressure, and pressure is where the truth lives.</Takeaway>
...
On a transit engagement we followed a single workflow at click level, the way you read a system rather than a model. Somewhere in the middle of it sat a piece of software costing forty thousand pounds a year that two people used to shuttle data between two other tools by hand. Nobody had questioned it. It was on no slide. It was just the shortcut that had hardened into a habit, and then into a line item.

We retired the licence, kept one tool, and moved the work to two internal builders. The operating model never mentioned any of it. The system was the only place that cost was visible, and the only place the saving was.
</FieldNote>
...
<ProcessFlow
  stages={[
    { label: "Pick one process", body: "Choose a single process that matters and that people complain about. Not the whole function. One flow, end to end.", meta: "Day 1" },
    { label: "Follow the work, not the chart", body: "Trace one real item through it: a new invoice, a new hire, a complaint. Note every hand-off and every tool it touches.", meta: "Days 2-4" },
    { label: "Find the workarounds", body: "Where does the official route get abandoned because it is too slow to survive a deadline? That shortcut is the system.", meta: "Days 5-7" },
    { label: "Name the gap out loud", body: "Write down where the model and the system diverge. That divergence is the unit of work, not the model itself.", meta: "Week 2" },
  ]}
/>
...
<DecisionFilter
  title="Run any process you care about through this"
  criteria={[
    { test: "Where was the decision actually made, and when?", fail: "The official meeting on the official date is the honest answer for no one" },
    { test: "What workaround exists because the official route is too slow to survive a real deadline?", fail: "There is one, you just have not asked the person who uses it" },
    { test: "Who fixes this off the record when it breaks on a Friday?", fail: "A name comes up that appears nowhere on the org chart" },
    { test: "If you deleted the slide tomorrow, what would change on Monday?", fail: "Nothing, which tells you the slide was never running the work" },
  ]}
  verdict="If you can only answer for the model, that is the finding, not a criticism. The system is already there doing the real work. Go and read it before you decide what to build on top."
/>
...
<KeyTakeaways>
- Models describe, systems execute. Conflating them is the most common and most expensive consulting failure there is.
- The gap between the model and the system is the real unit of work for an operating leader, not the model itself.
- Read the system before you redesign the model. A model built on an unread system just stacks a second fiction on the first.
- The AI version is the same trap: a strategy deck earns budget, but only a running workflow earns the reclaimed hours.
- Take one process that matters, follow the real work through it, and name where intent and execution diverge. That is the whole job, repeated.
</KeyTakeaways>
