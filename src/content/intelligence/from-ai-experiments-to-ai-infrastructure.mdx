export const frontmatter = {
  ...
  description: "Move from AI pilot to production when workflows repeat, controls are clear and shared infrastructure stops People and ops teams rebuilding the same plumbing.",
  keywords: ["AI adoption in HR", "move from AI pilot to production", "AI infrastructure", "AI experiments", "scaling AI pilots", "AI adoption strategy"],
  readTime: "11 min",
...
export const faqs = [
  ...
  },
  {
    ...
    answer:
      "Far less than the moonshot platform the pitch deck implies. The substrate is data access you mostly already have, a workflow tool at roughly £20 per builder seat per month, and a governance page in Notion. The real cost is deciding which workflow needs it first and giving one operating leader the time to own it. Budget weeks of build for the first pillar, not a quarter.",
  },
];
...
Move from AI pilot to production when the same workflow has been built three times, or one experiment creates value in more than one function. At that point, shared data access, tools, governance and maintenance stop being overhead. They are the infrastructure that prevents every team rebuilding the same plumbing.
...
| Tools | A small set of stable interfaces any new workflow can call without re-negotiating access | A workflow tool, roughly £20 per builder seat per month, SOC 2 and ISO 27001, self-hostable | A fortnight for the first, minutes after |
