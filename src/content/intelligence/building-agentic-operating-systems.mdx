...
export const faqs = [
  {
    ...
    answer:
      "You'll know you need one the first time a second agent breaks something the first agent quietly depended on. That's the signal: shared tools, shared memory, and shared policy stop being nice-to-haves and start being the only thing standing between you and a production incident. Most companies build the definition after the incident, not before.",
  },
  {
    question: "How is agentic AI different from prompting an LLM?",
    answer:
      "Cost and blast radius are the practical difference. A bad prompt wastes one model call and a person's patience. A bad agent can call ten tools, write to three systems, and be three steps into a mistake before anyone notices. Budget and monitor agents like a junior hire with API keys, not like a chatbot.",
  },
  {
    question: "What are agentic AI services?",
    answer:
      "Most teams buy the wrong end of the list first: a hosted agent framework, before they have a tool gateway or a policy service to plug it into. The framework then has nothing safe to call. Build the boring internal services, retrieval, identity, tool gateway, before you shop for the exciting orchestrator on top.",
  },
  {
    question: "Do I need a multi-agent system or one big agent?",
    answer:
      "Multi-agent earns its complexity once you have two workflows that are each independently boring and reliable, and the handoff between them mirrors a split that already exists in your org chart. If humans don't already divide the work that way, a multi-agent system won't invent the division for you. It'll just add a second thing that can fail.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "In the order they actually bite, not the order a vendor deck presents them: no observability first, because you ship blind and can't tell what broke. No tool design second, because the agent flails with too many options. No human-in-the-loop third, because that flailing eventually costs something real. Fix them in that order.",
  },
];
